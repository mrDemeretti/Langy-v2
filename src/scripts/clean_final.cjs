    ...
    fileEdits[f].forEach(edit => {
        ...
        for (let char of Object.keys(map)) {
            if (newLine.includes(char)) {
                let icon = map[char];
                // Replace logic depending on formatting
                
                // >🎉< -> >${LangyIcons.sparkles}<
                let f1 = ">" + char + "<";
                let t1 = ">${LangyIcons." + icon + "}<";
                newLine = newLine.split(f1).join(t1);
                
                // '<span class="...">' + char + ' Earned!' -> '\`<span class="...">\${LangyIcons.X} Earned!\`'
                if (newLine.includes(char + " Earned!")) {
                    newLine = newLine.split("'" + char + " Earned!'").join("\`\${LangyIcons." + icon + "} Earned!\`");
                }
                ...
                // emoji: '🎉' -> emoji: LangyIcons.sparkles 
                newLine = newLine.split("emoji: '" + char + "'").join("icon: LangyIcons." + icon);
                newLine = newLine.split("icon: '" + char + "'").join("icon: LangyIcons." + icon);
                
                // 'Address copied! 📋' -> `Address copied! ${LangyIcons.clipboard}`
                if (newLine.includes(".showToast('") && newLine.includes(char)) {
                    newLine = newLine.replace(/'([^']*)'/g, (m, str) => {
                        if(str.includes(char)) {
                            return "\`" + str.replace(char, "\${LangyIcons." + icon + "}") + "\`";
                        }
                        return m;
                    });
                }
                
                // General text within HTML if it still exists (e.g. <span>⏰ ${event.timeLeft} left</span>)
                if (newLine.includes(char) && !newLine.includes("console.")) {
                     newLine = newLine.replace(char, "\${LangyIcons." + icon + "}");
                     
                     // Force string wrapping if it's inside single quotes
                     if (newLine.includes("'") && newLine.includes("\${LangyIcons")) {
                          // Simple heuristic: if line contained '...${LangyIcons.X}...' it needs backticks.
                          // We'll replace the outer quotes if it matches.
                          newLine = newLine.replace(/'([^']*)\$\{LangyIcons\.([a-zA-Z]+)\}([^']*)'/g, "\`$1\${LangyIcons.$2}$3\`");
                     }
                }
            }
        }
