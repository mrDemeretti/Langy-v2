                    ...
                    <button id="nav-learning" class="btn btn--primary btn--xl btn--full" style="font-size: var(--fs-lg); display: flex; align-items: center; justify-content: center; gap: var(--sp-2); flex-direction: column; padding: 14px 24px;">
                        ${(() => {
                            if (isTrueBeginner) {
                                ...
                                <div style="font-size:var(--fs-xs); opacity:0.8; font-weight:var(--fw-medium);">${LangyState.progress.currentUnit || ({ en: 'Learn key phrases before speaking', ru: 'Выучи ключевые фразы перед разговором', es: 'Aprende frases clave antes de hablar' }[lang])}</div>`;
                            } else if (isPostLessonBeginner) {
                                ...
                                return `<div style="display:flex; align-items:center; gap:var(--sp-2);"><span style="font-size: 22px; display:flex;">${LangyIcons.rocket}</span> ${i18n('home.continue')}</div><div style="font-size:var(--fs-xs); opacity:0.8; font-weight:var(--fw-medium);">${LangyState.progress.currentUnit || i18n('learn.next_lesson')}</div>`;
                            }
                        })()}
                    </button>
                </div>
