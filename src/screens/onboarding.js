...
function renderOnboarding(container) {
    ...
    if (step === 1) {
        const interests = [
            ...
            { id: 'work', emoji: '💼', label: 'Работа и карьера' },
            { id: 'movies', emoji: '🎬', label: 'Кино и сериалы' },
            { id: 'gaming', emoji: '🎮', label: 'Игры' },
            { id: 'music', emoji: '🎵', label: 'Музыка' },
            { id: 'tech', emoji: '💻', label: 'IT и технологии' },
            { id: 'sports', emoji: '⚽', label: 'Спорт' },
            { id: 'food', emoji: '🍕', label: 'Еда и кулинария' },
            { id: 'books', emoji: '📚', label: 'Книги' },
            { id: 'social', emoji: '💬', label: 'Общение с иностранцами' },
            { id: 'exams', emoji: '🎓', label: 'Экзамены (IELTS, TOEFL)' },
            { id: 'business', emoji: '📊', label: 'Бизнес-английский' },
        ];
    ...
    if (step === 2) {
        const mascots = [
            { id: 0, name: 'Zendaya', emoji: '🌸', desc: 'Яркая и вдохновляющая. Делает обучение стильным и увлекательным!', style: 'Трендовый наставник', file: 'zendaya' },
            { id: 1, name: 'Travis Scott', emoji: '🌵', desc: 'Энергичный и харизматичный. Учит через музыку и культуру.', style: 'Культурный трекер', file: 'travis' },
            { id: 2, name: 'Matthew', emoji: '🎬', desc: 'Спокойный и уверенный. Объясняет всё чётко и с шармом.', style: 'Голливудский класс', file: 'matthew' },
            { id: 3, name: 'Omar', emoji: '😎', desc: 'Мудрый и харизматичный. Учит через жизненные истории.', style: 'Мудрый друг', file: 'omar' },
        ];
                ...
                <div class="onboarding__mascots" id="mascot-grid">
                    ${mascots.map(m => `
                        <div class="mascot-card ${selectedMascot === m.id ? 'mascot-card--selected' : ''}" data-id="${m.id}">
                            ...
                            ${selectedMascot === m.id ? '<div class="mascot-card__check">✓</div>' : ''}
                        </div>
                    `).join('')}
                </div>
        ...
        container.querySelector('#onboarding-finish').addEventListener('click', () => {
            ...
            Anim.showToast(`${['Zendaya', 'Travis Scott', 'Matthew', 'Omar'][selectedMascot]} будет твоим учителем! 🎉`);
